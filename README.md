...
推荐运行 `python main.py`，在主菜单选择“管理 · 业务组”。菜单支持查看、新增、修改和删除业务组；添加应用时可以用上下方向键移动蓝色焦点、空格多选、回车统一确认，已选项会显示为绿色。删除本地业务组或移除组内应用不会删除 Dify Console 中的真实应用。
...
所有单选菜单同时支持方向键和数字输入：上下键移动焦点，输入编号会立即将焦点同步到对应项，按回车确认。非交互终端仍使用传统数字输入方式。
